...
const OPTIMIZATION_CONFIGS: Record<string, OptimizationConfig> = {
  // Logos and icons - very aggressive compression, small size
  ...
  // OG image - balanced compression
  'og-image.png': { quality: 75, maxWidth: 1200, maxHeight: 630 },
  
  // Profile photo
  'profilepic.png': { quality: 75, maxWidth: 400, maxHeight: 400 },
  
  // Screenshots - moderate compression
  'maganji-screenshot.png': { quality: 70, maxWidth: 800, maxHeight: 600 },
  'tuandike-screenshot.png': { quality: 70, maxWidth: 800, maxHeight: 600 },
  
  // Service screenshots
  'dental-ai-chatbot-screenshot.png': { quality: 65, maxWidth: 800, maxHeight: 600 },
  'fleet-ai-dashboard-screenshot.png': { quality: 65, maxWidth: 800, maxHeight: 600 },
  'ai-voice-caller-screenshot.png': { quality: 65, maxWidth: 800, maxHeight: 600 },
  'ai-market-research-screenshot.png': { quality: 65, maxWidth: 800, maxHeight: 600 },
  'ai-dental-marketing-ad.png': { quality: 65, maxWidth: 800, maxHeight: 600 },
};

async function optimizeImage(inputPath: string, outputPath: string, config: OptimizationConfig): Promise<void> {
...
async function optimizeDirectory(dirPath: string): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      console.log(`✓ ${entry.name}: ${(beforeStats.size / 1024).toFixed(1)}KB → ${(afterStats.size / 1024).toFixed(1)}KB (${savings}% smaller)`);
    } catch (error) {
...
async function main(): Promise<void> {
  ...
  for (const imgName of rootImages) {
    ...
    try {
      ...
      const savings = ((1 - afterStats.size / beforeStats.size) * 100).toFixed(1);
      console.log(`✓ ${imgName}: ${(beforeStats.size / 1024).toFixed(1)}KB → ${(afterStats.size / 1024).toFixed(1)}KB (${savings}% smaller)`);
    } catch (error) {
