  ...
  return (
    <div className="relative w-[350px] sm:w-[400px] shrink-0 p-6 sm:p-8 rounded-2xl flex flex-col gap-4 backdrop-blur-xl bg-black/40 border border-white/10 shadow-2xl transition-transform duration-300 hover:scale-[1.02] cursor-default">
      ...
      <div className="pt-4 border-t border-white/5 flex flex-col">
        <span className="font-semibold text-white tracking-wide text-sm">{item.name} · {item.role}</span>
        <div className="flex justify-between items-center mt-1">
          <span className="text-xs text-indigo-400/80">{item.location}</span>
          <span className="text-xs font-mono bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded border border-indigo-500/30">{item.metric}</span>
        </div>
      </div>
    </div>
  );
